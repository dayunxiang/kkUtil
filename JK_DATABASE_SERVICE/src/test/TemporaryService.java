...
public class TemporaryService implements Runnable {
...
//			"yyyy-MM-dd HH:mm:ss");
	...
	public TemporaryService(DataBaseConnection config) {
		this.config = config;
		getConn();
	}
		...
		while (calendar.get(Calendar.HOUR_OF_DAY) == hour) {
			...
			if (remark_text == null)
				insertSQL = "INSERT INTO "
						...
						+ (sTime + config.getInterval() * 60) + "'," + MyUtil.getString(data) + ")";
			else
				insertSQL = "INSERT INTO "
						+ config.getTable_d()
						+ "("
						...
						+ ",remark) VALUES('" + (sTime + config.getInterval() * 60) + "',"
...
//				connection.commit();
	...
	// if (!getConn())
	// return;
	// System.out.println("operationByGroup");
